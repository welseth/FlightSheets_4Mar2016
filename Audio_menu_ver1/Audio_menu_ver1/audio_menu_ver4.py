...
#Generic test to see if the speaker works "If you can hear this"
print ("Playing test sound, please wait...")
os.system('amixer set PCM -- 85%')  #set volume of RasPI to 98% for normal use, (testing is 85%)
...
def what_mode(channel):
    print("Programming mode: ",programming_mode)
    print("playpback_songs_mode:  ",playback_songs_mode)
    operational_mode = -1
    ...
    print("Operational_mode:  ", operational_mode, "\n\n")
    what_function(channel)
...
def what_function(channel):
    ...
    #>>> ANNOUNCE the menu HERE <<<
        ...
        #different actions
    print("Value of speak_menu:  ", speak_menu, "\n\n")
...
def play_song_one(channel):  
    ...
    if GPIO.input(5):       #if port == 1
        ...
        player_one.stdin.flush()
        print ("Pin 5/Switch 1 High transition- Sent play/pause to process. Timestamp:", time.strftime("%H:%M:%S"))  #print time for debugging only
        #look_for_end()
    else:                   #if port == 0
        player_one.stdin.flush()
        player_one.stdin.write(b'p\n')
...
def play_song_two(channel):
    print ("Pin 6/Switch 2 Song Two will eventually go here.  ", channel)
...
#***** Pin 12 *****  program_switch   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<fix this <<<
def program_mode_switch_handler(channel):
    print ("\n\n\nPin 12/Toggle Switch Now in program_switch mode...")
    program_switch = programming_mode
    print ("Program_switch status: ", program_switch, "\n\n\n")
    what_mode(channel)     #go check for proper response to the program swich change of state
...
GPIO.add_event_detect(12, GPIO.BOTH, callback=program_mode_switch_handler, bouncetime=50) #program_switch
...
try:  
    ...
    player_one.stdin.flush()
    player_one.stdin.write(b'VOLUME 60\n')  #turn up volume 100=all the way, smaller is "less volume"
    player_one.stdin.flush()
    player_one.stdin.write(b'LOADPAUSED /home/pi/sounds/boys_round_here.mp3\n')
    player_one.stdin.flush()
